...
const ProfilePage = () => {
  ...
  const onSubmit = async (data: ProfileForm) => {
    ...
    try {
      ...
      navigate('/dashboard/minhas-simulacoes'); 
    } catch (error) {
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Precisamos de alguns dados para encontrar a melhor proposta para você.</p>
        </div>
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="relative">
                            <Input {...field} placeholder="00000-000" maxLength={9} onBlur={(e) => { field.onBlur(); handleCepBlur(e.target.value); }} />
                            {isCepLoading && <Loader2 className="absolute right-2 top-2 h-5 w-5 animate-spin text-muted-foreground" />}
                          </div>
                        </FormControl><FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="street" render={({ field }) => (<FormItem className="md:col-span-2"><FormLabel>Endereço *</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
